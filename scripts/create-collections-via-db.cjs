// Create collections via direct database connection
const { spawn } = require('child_process');
const fs = require('fs');
const path = require('path');

async function createCollectionsViaDB() {
  try {
    console.log('🔧 Creating collections via database connection...\n');
    
    // Create SQL script for collections
    const sqlScript = `
...
`;
    ...
    const sqlFile = path.join(__dirname, 'create-collections.sql');
    fs.writeFileSync(sqlFile, sqlScript);
    console.log('   ✅ SQL script created');
    
    // Execute SQL script via Docker
    console.log('\n2. Executing SQL script via Docker...');
    return new Promise((resolve, reject) => {
      const dockerProcess = spawn('docker', [
        'exec', '-i', 'saloonmarketplace-database-1',
      ...
      dockerProcess.stdin.write(sqlScript);
      ...
      dockerProcess.on('close', (code) => {
        if (code === 0) {
          ...
          console.log('   ❌ SQL script failed with code:', code);
    ...
    console.error('❌ Create collections via DB failed:', error);
    return false;
  }
}
...
async function main() {
  try {
    console.log('🚀 Creating collections and running seeder...\n');
    
    // Create collections via database
    const collectionsCreated = await createCollectionsViaDB();
    
    if (collectionsCreated) {
      console.log('\n3. Waiting for Directus to recognize collections...');
      ...
      try {
        const { spawn } = require('child_process');
        const permProcess = spawn('node', ['scripts/fix-final-permissions.cjs'], {
          cwd: process.cwd(),
          stdio: 'inherit'
        });
        
        permProcess.on('close', (code) => {
          if (code === 0) {
            console.log('\n5. Running original seeder...');
            const seederProcess = spawn('node', ['tests/scripts/seeder.js'], {
              cwd: process.cwd(),
              stdio: 'inherit'
            });
            
            seederProcess.on('close', (seederCode) => {
              if (seederCode === 0) {
                console.log('\n🎉 Everything completed successfully!');
                console.log('\n📋 Summary:');
                console.log('   ✅ Collections created via database');
                console.log('   ✅ Permissions set correctly');
                console.log('   ✅ Seeder populated data');
                console.log('   ✅ Frontend should work without errors');
                
                console.log('\n🚀 Next Steps:');
                console.log('   1. Test frontend: http://localhost:3000');
                console.log('   2. Check Directus Admin: http://localhost:8055/admin');
                console.log('   3. Verify data and images');
              } else {
                console.log('\n❌ Seeder failed');
              }
            });
