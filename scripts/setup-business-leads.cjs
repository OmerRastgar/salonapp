...
async function setupBusinessLeads() {
  try {
    ...
    // Create business leads table directly in PostgreSQL
    console.log('\n2. Creating business leads table...');
    try {
      const createTableSQL = `
        ...
        // Execute via Directus raw SQL endpoint
        ...
        if (sqlResponse.ok) {
          console.log('   ✅ Business leads table created');
        } else {
          console.log('   ⚠️  Table creation may have failed, continuing...');
        }
      } catch (error) {
        console.log('   ⚠️  Table creation error, continuing...');
      }
    
    // Create collection in Directus
    console.log('\n3. Creating Directus collection...');
    try {
    ...
    if (publicPolicyId && adminPolicyId) {
      ...
      for (const permission of permissions) {
        try {
          ...
          if (permResponse.ok) {
            console.log(`   ✅ Created permission: ${permission.action} for ${permission.policy === publicPolicyId ? 'Public' : 'Administrator'}`);
          } else {
    ...
    console.log('\n7. Testing business leads creation...');
    try {
      const testLead = {
