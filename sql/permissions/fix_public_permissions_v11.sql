-- Fix Public Permissions for Directus 11
-- This script creates the public role and policy with correct column names

BEGIN;

-- First, let's see what roles exist and create the Public role if needed
DO $$
DECLARE
    public_role_id UUID;
...
BEGIN
    ...
    IF public_role_id IS NULL THEN
        ...
        -- Directus 11 uses different column names
        INSERT INTO directus_roles (id, name, icon, description) 
        VALUES (gen_random_uuid(), 'Public', 'public', 'Public access for unauthenticated users')
    ...
    INSERT INTO directus_policies (id, name, icon, description, users, roles, ip_addresses) 
    VALUES (
        gen_random_uuid(),
        'Public Access',
        'public',
